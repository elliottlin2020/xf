...
public class DepartmentAuthorityQueryService implements QueryService<DepartmentAuthority> {
    ...
    protected QueryWrapper<DepartmentAuthority> createQueryWrapper(DepartmentAuthorityCriteria criteria) {
        QueryWrapper<DepartmentAuthority> queryWrapper = new QueryWrapper<>();
        if (criteria != null) {
            if (StringUtils.isNotEmpty(criteria.getJhiCommonSearchKeywords())) {
                ...
                if (criteria.getCreateUserId() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getCreateUserId(), "create_user_id"));
                }
                if (criteria.getCreateTime() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getCreateTime(), "create_time"));
                }
                ...
                if (criteria.getDepartmentId() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getDepartmentId(), "department_id"));
                }
                if (criteria.getDepartmentName() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getDepartmentName(), "department_left_join.name"));
                }
            }
        }
        return queryWrapper;
     ...
     * @param pageable 分页
     * @return Page<DepartmentAuthorityDTO>
     */
    @Transactional(readOnly = true)
    public Page<DepartmentAuthorityDTO> selectByCustomEntity(
        String entityName,
        ...
        if (StringUtils.isEmpty(entityName)) {
            entityName = "DepartmentAuthority";
        }
        ...
        // DynamicJoinQueryWrapper<DepartmentAuthority, DepartmentAuthority> dynamicJoinQueryWrapper = new DynamicJoinQueryWrapper<>(DepartmentAuthority.class, dynamicRelationships);
        ...
        List<DepartmentAuthorityDTO> result = dynamicJoinQueryWrapper
            .queryList(DepartmentAuthority.class, pagination)
            .stream()
            .peek(Binder::bindRelations)
            .map(departmentAuthorityMapper::toDto)
            .collect(Collectors.toList());
        return new Page<DepartmentAuthorityDTO>(pagination.getPageIndex(), pagination.getPageSize(), pagination.getTotalCount())
            .setRecords(result);
    }
}
