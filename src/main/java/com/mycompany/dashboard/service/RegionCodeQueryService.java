...
public class RegionCodeQueryService implements QueryService<RegionCode> {
        ...
        if (criteria != null) {
            if (StringUtils.isNotEmpty(criteria.getJhiCommonSearchKeywords())) {
                ...
                if (criteria.getAreaCode() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getAreaCode(), "area_code"));
                }
                if (criteria.getCityCode() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getCityCode(), "city_code"));
                }
                if (criteria.getMergerName() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getMergerName(), "merger_name"));
                }
                if (criteria.getShortName() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getShortName(), "short_name"));
                }
                if (criteria.getZipCode() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getZipCode(), "zip_code"));
                }
                if (criteria.getLevel() != null) {
                    queryWrapper = queryWrapper.and(buildSpecification(criteria.getLevel(), "level"));
                }
                if (criteria.getLng() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getLng(), "lng"));
                }
                if (criteria.getLat() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getLat(), "lat"));
                }
                ...
                if (criteria.getParentName() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getParentName(), "region_code_left_join.name"));
                }
            }
        }
        return queryWrapper;
     ...
     * @param pageable 分页
     * @return Page<RegionCodeDTO>
     */
    @Transactional(readOnly = true)
    public Page<RegionCodeDTO> selectByCustomEntity(
        String entityName,
        ...
        // DynamicJoinQueryWrapper<RegionCode, RegionCode> dynamicJoinQueryWrapper = new DynamicJoinQueryWrapper<>(RegionCode.class, dynamicRelationships);
        ...
        List<RegionCodeDTO> result = dynamicJoinQueryWrapper
            .queryList(RegionCode.class, pagination)
            .stream()
            .peek(Binder::bindRelations)
            .map(regionCodeMapper::toDto)
            .collect(Collectors.toList());
        return new Page<RegionCodeDTO>(pagination.getPageIndex(), pagination.getPageSize(), pagination.getTotalCount()).setRecords(result);
    }
}
