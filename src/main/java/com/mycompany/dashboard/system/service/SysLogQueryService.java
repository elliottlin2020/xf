...
public class SysLogQueryService implements QueryService<SysLog> {
    ...
    public SysLogQueryService(SysLogRepository sysLogRepository, CommonTableRepository commonTableRepository, SysLogMapper sysLogMapper) {
        ...
        if (criteria != null) {
            if (StringUtils.isNotEmpty(criteria.getJhiCommonSearchKeywords())) {
                ...
                if (criteria.getLogType() != null) {
                    queryWrapper = queryWrapper.and(buildSpecification(criteria.getLogType(), "log_type"));
                }
                if (criteria.getLogContent() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getLogContent(), "log_content"));
                }
                if (criteria.getOperateType() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getOperateType(), "operate_type"));
                }
                if (criteria.getUserid() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getUserid(), "userid"));
                }
                if (criteria.getUsername() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getUsername(), "username"));
                }
                if (criteria.getIp() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getIp(), "ip"));
                }
                ...
                if (criteria.getRequestUrl() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getRequestUrl(), "request_url"));
                }
                if (criteria.getRequestType() != null) {
                    queryWrapper = queryWrapper.and(buildStringSpecification(criteria.getRequestType(), "request_type"));
                }
                if (criteria.getCostTime() != null) {
                    queryWrapper = queryWrapper.and(buildRangeSpecification(criteria.getCostTime(), "cost_time"));
                }
     ...
     */
    @Transactional(readOnly = true)
    public Page<SysLogDTO> selectByCustomEntity(String entityName, SysLogCriteria criteria, QueryWrapper queryWrapper, Page pageable) {
        ...
        // DynamicJoinQueryWrapper<SysLog, SysLog> dynamicJoinQueryWrapper = new DynamicJoinQueryWrapper<>(SysLog.class, dynamicRelationships);
        ...
        List<SysLogDTO> result = dynamicJoinQueryWrapper
            .queryList(SysLog.class, pagination)
            .stream()
            .peek(Binder::bindRelations)
            .map(sysLogMapper::toDto)
            .collect(Collectors.toList());
        return new Page<SysLogDTO>(pagination.getPageIndex(), pagination.getPageSize(), pagination.getTotalCount()).setRecords(result);
    }
}
