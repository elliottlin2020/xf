...
import com.mycompany.dashboard.system.service.SmsMessageQueryService;
import com.mycompany.dashboard.system.service.SmsMessageService;
 ...
 * 管理实体{@link com.mycompany.dashboard.system.domain.SmsMessage}的REST Controller。
 */
@RestController
@RequestMapping("/api")
...
public class SmsMessageResource {
    ...
    public ResponseEntity<SmsMessageDTO> updateSmsMessage(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, smsMessageDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<SmsMessageDTO> partialUpdateSmsMessage(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, smsMessageDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<SmsMessageDTO>> getAllSmsMessages(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = smsMessageQueryService.selectByCustomEntity(listModelName, criteria, queryWrapper, PageableUtils.toPage(pageable));
            } else {
                page = smsMessageQueryService.selectByCustomEntity(listModelName, criteria, null, PageableUtils.toPage(pageable));
            }
        } else {
            if (commonQueryId != null) {
                QueryWrapper queryWrapper = commonConditionQueryService.createQueryWrapper(commonQueryId);
                page = smsMessageQueryService.findByQueryWrapper(queryWrapper, PageableUtils.toPage(pageable));
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        IPage<SmsMessageDTO> page = smsMessageService.findAll(new Page<>(1, Integer.MAX_VALUE));
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), SmsMessageDTO.class, page.getRecords());
    ...
    public ResponseEntity<Void> exportToExcel(MultipartFile file) throws IOException {
        ...
        List<SmsMessageDTO> list = ExcelImportUtil.importExcel(savedFile, SmsMessageDTO.class, params);
    ...
    public ResponseEntity<SmsMessageDTO> updateSmsMessageBySpecifiedFields(
        @RequestBody SmsMessageDTOAndSpecifiedFields smsMessageDTOAndSpecifiedFields
    ) throws URISyntaxException {
        log.debug("REST request to update SmsMessage : {}", smsMessageDTOAndSpecifiedFields);
    ...
    /**
     * {@code PUT  /sms-messages/specified-field} : Updates an existing smsMessage by specified field.
     *
     * @param smsMessageDTOAndSpecifiedFields the smsMessageDTO and specifiedFields to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated smsMessageDTO,
     * or with status {@code 400 (Bad Request)} if the smsMessageDTO is not valid,
