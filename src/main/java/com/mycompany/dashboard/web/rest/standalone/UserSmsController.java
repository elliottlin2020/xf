package com.mycompany.dashboard.web.rest.standalone;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.mycompany.dashboard.domain.User;
import com.mycompany.dashboard.security.SecurityUtils;
import com.mycompany.dashboard.security.UserNotActivatedException;
...
import com.mycompany.dashboard.sms.builder.SmsBuilder;
import com.mycompany.dashboard.sms.model.SmsCode;
import com.mycompany.dashboard.sms.model.SmsData;
import com.mycompany.dashboard.web.rest.vm.LoginMobileVM;
import java.util.*;
...
public class UserSmsController {
    ...
    public ResponseEntity<JWTToken> authorizeByMobile(@Valid @RequestBody LoginMobileVM loginMobileVM) {
        ...
        // Result<Void> result = daYuSmsService.verify(loginMobileVM.getMobile(), loginMobileVM.getCode(), loginMobileVM.getImageCode(), "Dashboard");
        if (result) {
            // 通过mobile查找用户信息。
    ...
    public ResponseEntity<Result<String>> getSmsCodeByCurrentUser(String mobile, @RequestParam(name = "imageCode", required = false) String imageCode) {
        ...
        if (SecurityUtils.getCurrentUserLogin().isPresent()) {
            ...
            if (!existsMobile) {
                // 不存在，发送验证码
                Result<String> result = daYuSmsService.sendVerifyCode(mobile, "Dashboard");
